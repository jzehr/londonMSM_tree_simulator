# time origin around 1955, higher import rate
...
year0 <- 1955
year1 <- 2013
time_res <-  12 * (year1 - year0 )  # time steps / week #1e4 #1e3 
date0 <- as.Date('1955-01-01')
date1 <- as.Date('2012-12-31')
times0 <- 0
times1 <- as.numeric( date1 - date0 )
times_year <- seq(year0, year1, length.out = time_res) #to end of 2012
...
dydt <- function(t,y, parms, ... ){
	y <- pmax(y, 0 )
	if ( days2years( t ) < 1979 ){
		dy <- setNames( rep(0, m ), DEMES )
